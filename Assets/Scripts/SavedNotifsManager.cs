...
public class SavedNotifsManager : MonoBehaviour
{
    ...
    void loadPersistent()
    {
        for (int i = 0; i < SaveData.savedNotifs.Count; i++)
        {
            ...
            Davinci.get().load("https://YOURSERVERIPANDPORTHERE" + SaveData.savedNotifs[i].appID + ".png").setFadeTime(1).setCached(true).into(NotifObject.GetComponentInChildren<Image>()).start();
