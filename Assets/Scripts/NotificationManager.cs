...
public class NotificationManager : MonoBehaviour
{
    ...
    {
        StartCoroutine(Post("https://YOURSERVERIPANDPORTHERE", "{\"type\": \"token\", \"token\": \"" + token.Token + "\"}"));
    }
    ...
    public void OnMessageReceived(object sender, Firebase.Messaging.MessageReceivedEventArgs e) //When a message is sent downstream from the prodigy server.
    {
        ...
        if (data["type"] == "batt") batteryTexts[data["device"]].text = data["device"] + ":\n" + data["level"] + "%";
            ...
            Davinci.get().load("https://YOURSERVERIPANDPORTHERE" + data["app"] + ".png").setFadeTime(1).setCached(true).into(NotifObject.GetComponentInChildren<Image>()).start();
    ...
    void loadPersistent()
    {
        ...
        for (int i = 0; i < SaveData.notifs.Count; i++)
        {
            ...
            Davinci.get().load("https://YOURSERVERIPANDPORTHERE" + SaveData.notifs[i].appID + ".png").setFadeTime(1).setCached(true).into(NotifObject.GetComponentInChildren<Image>()).start();
    ...
    {
        StartCoroutine(Post("https://YOURSERVERIPANDPORTHERE", "{\"type\": \"fakenotif\"}"));
    }
    public void DeleteRipple(GameObject delete, string reason)
    {
        ...
        {
            NotificationObject notifData = SaveData.notifs.Find(lookForItem);
            StartCoroutine(Post("https://YOURSERVERIPANDPORTHERE", "{\"type\": \"block\", \"app\": \"" + notifData.appID + "\"}"));
            delete.GetComponent<Animation>().clip = delete.GetComponent<Animation>().GetClip("SwipeRight");
            StartCoroutine(playAnim(delete.GetComponent<Animation>(), reason));
        }
